...
my $nasne_addr = '192.168.21.10';
my $datafile    = '/tmp/zabbix_nasne.dat';
my $curl = 'curl -s "http://".$nasne_addr.":64210/status/boxStatusListGet"';
...
my $curl = 'curl -s "http://'.$nasne_addr.':64210/status/channelInfoGet2?networkId='.$network.'&transportStreamId='.$transport.'&serviceId='.$service.'&withDescriptionLong=0"';
...
if ($dat ne $log && $log) {
	# sender
	system("zabbix_sender -z <zabbix_host> -s <server_name> -k nasne_rec_log -o \"$log\"");
